...
function renderPreviewHTML(data, content, slug, { saved = false, rawBody = null } = {}) {
  ...
  const tags    = data.tags?.length
    ...
    : '';
  const coverImg = data.cover && slug
    ? `<img src="${esc(coverUrl(slug, data.cover))}" alt="cover"
            style="float:right;max-width:200px;margin:0 0 1rem 1.5rem;border-radius:8px">`
    : '';
